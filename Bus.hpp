...
{
public:
	BusErr Init(int32_t shmkey, int32_t shmSize, bool master)
	{
	    int shmid = 0;
	    ...
	    if (master)
	    {
	        ...
	        recv.Init(busBuf, recvBufLen, true);
	        send.Init(busBuf + recvBufLen, sendBufLen, true);
	    }
	    else
	    {
	        int32_t sendBufLen = shmSize / 2;
	        int32_t recvBufLen = shmSize - sendBufLen;
	        send.Init(busBuf, sendBufLen, false);
	        recv.Init(busBuf + sendBufLen, recvBufLen, false);
	    }
	...
	{
		return recv.Read(data, bufLen, readSize);
	}
